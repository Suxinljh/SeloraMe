 ...
 * 评测作答会话。
 *
 * 每个评测一份，按 `selorme-session-<scaleId>` 存储，避免为每个评测写一套读写函数。
 * answers 的长度必须与评测的题目数一致；长度不符时视为无效数据并重置，
 * 以兼容评测题目变更后的旧数据。
 *
