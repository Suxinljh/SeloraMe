...
export default function AssessmentDetail () {
  ...
  useDidShow(() => setIsFavorite(scale ? isFavoriteAssessment(scale.id) : false))
  ...
  const start = () => Taro.navigateTo({ url: `/pages/questions/index?assessment=${scale.id}` })
  ...
  return <View className='assessment-detail-page'>
    ...
    <View className='assessment-detail-content'>
      <View className='assessment-detail-hero'>
        ...
        <Button className={`assessment-detail-favorite ${isFavorite ? 'is-favorite' : ''}`} onClick={toggleFavorite}>
          <Icon name='bookmark' variant={isFavorite ? 'round' : 'outlined'} className='assessment-detail-favorite-icon' />
          {isFavorite ? '已收藏' : '收藏测评'}
        </Button>
      ...
      <View className='assessment-detail-section'>
        <View className='assessment-detail-section-title'>
          <Icon name='psychology' className='assessment-detail-section-icon' />
          <Text>这是个什么样的测试？</Text>
        </View>
        {scale.intro.map((paragraph) => (
          <Text key={paragraph} className='assessment-detail-paragraph'>{paragraph}</Text>
        ))}
        {scale.highlights.length > 0 && <Text className='assessment-detail-highlights'>{scale.highlights}</Text>}
      </View>

      {scale.benefits.length > 0 && <View className='assessment-detail-section assessment-detail-source'>
        <View className='assessment-detail-section-title'>
          <Icon name='assessment' className='assessment-detail-section-icon' />
          <Text>这个测试有什么用？</Text>
        </View>
      ...
      {scale.instructions.length > 0 && <View className='assessment-detail-section'>
        <View className='assessment-detail-section-title'>
    ...
    <View className='assessment-detail-footer'>
      <Button className='assessment-detail-start' onClick={start}>立即开始</Button>
    </View>
  </View>
}
